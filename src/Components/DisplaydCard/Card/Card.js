...
function Card(props) {
    ...
    return (
        <Link to={ routes.LABORATOIRE+'/'+ props.slug } className={ classes.Card } >
            <article data-year={ props.date }>
                <h2 className={ classes.Article_title }>{ props.title }</h2>
                <div className={ classes.Article_content }>
                    <img src={ props.image } alt={ props.title } />
                </div>
                <div className={ classes.Article_footer }>
                    <p>{ maxLength(props.content, 35) }</p>
